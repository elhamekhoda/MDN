...
def main():
    ...
    '''
    trainY = gen_data(10000)
    validY = gen_data(1000)
    testY = gen_data(1000)

    trainX= gen_data(10000)
    validX= gen_data(1000)
    testX, = gen_data(1000)
    '''

    trainX_re = K.reshape(trainX, (-1,2))
    trainY_re = K.reshape(trainY, (-1,2))
    validX_re = K.reshape(validX, (-1,2))
    validY_re = K.reshape(validY, (-1,2))
    testX_re = K.reshape(testX, (-1,2))
    testY_re = K.reshape(testY, (-1,2))
    ...
    model = keras.models.Model(inputs=inputs, outputs=[keras.layers.Merge(mode='concat')([mixture_density(10)(h), mixture_density(10)(h)])])
    ...
    history = model.fit(
        ...
        batch_size=32,
        epochs=10,
        validation_data=(validX_re, validY_re),
        callbacks=[
