...
def mixture_density(nb_components, target_dimension=1):
    ...
    def layer(X):
        ...
        prec = keras.layers.Dense(nb_components, activation=K.exp)(X)
...
def mixture_density_loss(nb_components, target_dimension=1):
    ...
    def loss(y_true, y_pred):
        ...
        diff = K.reshape(y_true, (batch_size, 1, target_dimension)) - means


        diffsqSum = K.sum(diff*diff, axis=2)
        precssq = precs*precs
        expargs = -0.5*diffsqSum/precssq



        norms = np.power(sqrtof2pi*precs,target_dimension)* alphas

        logprob = - K.log(K.sum(norms * K.exp(expargs), axis=1))
...
def main():
    ...
    #h = keras.layers.Dense(300, activation='relu')(h)
    model = keras.models.Model(inputs=inputs, outputs=[mixture_density(10)(h)])
    ...
    model.compile(
        ...
        loss=mixture_density_loss(nb_components=10),
    )
    ...
    history = model.fit(
        ...
        callbacks=[
            keras.callbacks.ModelCheckpoint('mdn_2.h5', verbose=1, save_best_only=True)
        ],
        verbose=2
    )
    ...
    for i in range(y_pred.shape[0]):
        ...
        precs = y_pred[i,2*nb_components:3*nb_components]
